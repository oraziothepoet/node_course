  ...
  request(
    ...
    (error, { body }) => {
      if (error) {
        ...
        callback(
          undefined,
          `${body.current.weather_descriptions}. The temperature is ${body.current.temperature}, it feels like ${body.current.feelslike} and the humidity is ${body.current.humidity}%.`
        );
      }
    }
  );
